 ...
 * @Last Modified time: 2016-06-21 13:48:17
 */
...
switch ($step) {
    ...
    case 4:
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            $datafile = $setting['add_test'] == 1 ? './inc/newtengcms_data.sql' : './inc/newtengcms.sql';
            ...
            $result   = $db->exec("INSERT INTO `{$setting['DB_PREFIX']}admin` (`username`,`password`,`encrypt`,`usertype`,`logintime`,`loginip`,`islock`) VALUES ('{$setting['username']}','$password','$encrypt',9,'$time','$ip',0);");
            ...
            @file_get_contents('http://www.Newteng.com/api.php?c=Cms&a=getInstallInfo&email=' . base64_encode($setting['email']) . '&url=' . $_SERVER["HTTP_HOST"] . '&lang=' . $_SERVER['HTTP_ACCEPT_LANGUAGE']);
